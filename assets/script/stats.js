...
function renderTabla(arrayEvents) {
    let mayorCapacidad = arrayEvents.reduce((evento1, evento2) => {
        if (evento1.capacity > evento2.capacity) return evento1
        return evento2
    })
    console.log(mayorCapacidad);
    let mayorAssistance = arrayEvents.filter(eventos => eventos.assistance).reduce((evento1, evento2) => {
        if ((evento1.assistance / evento1.capacity) > (evento2.assistance / evento2.capacity)) return evento1
        return evento2
    })
    let menorAssistance = arrayEvents.filter(eventos => eventos.assistance).reduce((evento1, evento2) => {
        if ((evento1.assistance / evento1.capacity) < (evento2.assistance / evento2.capacity)) return evento1
        return evento2
    })
